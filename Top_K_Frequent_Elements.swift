...
// Solution swift Priority Queue

class Solution {
    typealias KFTuple = (num: Int, count: Int)

    func topKFrequent(_ nums: [Int], _ k: Int) -> [Int] {
        var res = [Int]()
        guard nums.count > 0 else { return res }
        var pq = PriorityQueue<KFTuple> { t1, t2 in return t1.count < t2.count }
        ...
        for (num, count) in dict {
            pq.enqueue((num: num, count: count))
            if pq.count > k {
                pq.dequeue()
            }
        }
        while !pq.isEmpty {
            res.append(pq.dequeue()!.num)
        }
        return res
    }
}
